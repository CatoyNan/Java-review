...
graph TB
	subgraph site
    pre-sit --> Site[site] --> post-site --> site-deploy
    end
    
    subgraph default
    compile --> test --> package打成jar --> install -->deploy
    end
    
    subgraph clean 
    pre-clean --> Clean[clean] --> post-clean
    end
```
...
- 若B在pom.xml文件中的位置先于D则C1.0将被引用，路径长度相同，先引入优先。
...
如果父亲项目和子module是同级目录，则module应这样修改
